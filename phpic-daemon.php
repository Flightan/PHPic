...
function getUserList ($dir, $path_users)
{
	$users = array();
...
function parseAlbumXML ($xml_album_path, $dir_user, $user_path)
{
	$exists_albums = array();
	...
	foreach($exists_albums as $al)
	{
		...
		if ($dir_album = opendir($album_path))
		{
			...
			if (!file_exists($xml_photo_path))
			{
				...
				prettySave($new_xml, $xml_photo_path);
...
$path_users = getcwd() . "/public/users/";

if ($dir = opendir($path_users))
{

	$users = getUserList($dir, $path_users);
	...
	foreach($users as $us)
	{
		...
		if ($dir_user = opendir($user_path))
		{
			...
			$xml_album_path = getcwd() . '/.config/album_'.$us.'.xml';
			$exists_albums = array();
			...
			if (!file_exists($xml_album_path))
			{
				...
				$new_xml = new SimpleXMLElement($root_node);
				prettySave($new_xml, $xml_album_path);
			}

			parseAlbumXML($xml_album_path, $dir_user, $user_path);
		}
	}
}